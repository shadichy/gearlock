...
PKG_KERNEL_IMAGE=$(grep -E "(kernel|vmlinu[zx])$" "/var/lib/gearlock/pkgs/$NAME/files")
...
if test -e "$FIRMDIR_UPDATE"; then
    rm -r "$FIRMDIR_UPDATE"
    handleError "Failed to cleanup $(basename "$FIRMDIR_UPDATE")"
fi
rm -f "${GBSCRIPT[1]}" "${GBSCRIPT[2]}"

# Remove kernel
kver=$(file -bL "$PKG_KERNEL_IMAGE" | grep -o 'version [^ ]*' | cut -d ' ' -f 2)
rm -rf "/boot/vmlinuz-$kver" "/boot/initramfs-$kver.img"
update-grub
...
if [ -d "$FIRMDIR_OLD" ] && [ "$(cat "$EFFECTIVE_FIRMDIR_PLACEHOLDER")" == "${NAME}_${VERSION}" ]; then
    while true; do
        ...
        [Yy])
            ...
            nout rm -r "$FIRMDIR" && mv "$FIRMDIR_OLD" "$FIRMDIR"
            handleError "Failed to restore stock $(basename "$FIRMDIR")"
            break
            ;;
