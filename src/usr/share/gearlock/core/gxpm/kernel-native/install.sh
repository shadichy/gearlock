...
[ -e "$PKG_KERNEL_IMAGE" ] || PKG_KERNEL_IMAGE="$BD"/system/lib/module/*/kernel
[ -e "$PKG_KERNEL_IMAGE" ] || PKG_KERNEL_IMAGE="$BD"/system/lib/module/*/vmlinu[zx]
...
doJob() {
    # Merge files
    gclone "$BD/system/" "$SYSTEM_DIR"
    handleError "Failed to place files"

    # Install new kernel image
    kver=$(file -bL "$PKG_KERNEL_IMAGE" | grep -o 'version [^ ]*' | cut -d ' ' -f 2)
    nout rsync "$PKG_KERNEL_IMAGE" "/boot/vmlinuz-$kver"
    handleError "Failed to update kernel image"
    mkinitfs "$kver"
    update-grub
...
while true; do
    ...
    case $i in
    [Yy])
        "$BD"/install_firmware.sh
        geco "\n\n+ Placing the kernel module and firmware files into your system"

        if [ "$TERMINAL_EMULATOR" == "yes" ]; then
            make_gbscript_UpdateFirmware
            mv "${BD}${FIRMDIR}" "${BD}${FIRMDIR_UPDATE}"
            handleError "Failed to rename package firmware to firmware.update"
            echo "${NAME}_${VERSION}" >"${BD}${FIRMDIR_UPDATE}/${EFFECTIVE_FIRMDIR_PLACEHOLDER}"
            doJob
            break
        else
            if [ -e "$FIRMDIR_OLD" ]; then
                nout rm -r "$FIRMDIR_OLD"
                handleError "Failed to cleanup firmware.old"
            fi
            if [ -e "$FIRMDIR" ]; then
                mv "$FIRMDIR" "$FIRMDIR_OLD"
                handleError "Failed to backup old firmware"
            fi
            doJob
            echo "${NAME}_${VERSION}" >"${FIRMDIR}/${EFFECTIVE_FIRMDIR_PLACEHOLDER}"
            break
        fi
        ;;
    [Nn])
        ...
        rm -f "${GBSCRIPT[1]}" "${GBSCRIPT[2]}"
        rm -rf "${BD}${FIRMDIR}"
        handleError "Failed to cleanup package firmware"
        doJob
        break
        ;;
