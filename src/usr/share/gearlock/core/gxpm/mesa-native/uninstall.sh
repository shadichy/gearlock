...
if [ "$TERMINAL_EMULATOR" == "yes" ]; then
	...
	while true; do
		...
		[Yy])
			...
			gsudo openvt -s gxpm -u "$UNINS_SCRIPT"
			return 101
			break
			;;
...
test -e "$MESA_BACKUP_FILE"
handleError "Mesa backup archive not found"
...
tar --zstd --strip-components=1 --warning=no-timestamp --directory="$SYSTEM_DIR" -xpf "$MESA_BACKUP_FILE"
handleError "Failed to restore stock Mesa from backup archive"
